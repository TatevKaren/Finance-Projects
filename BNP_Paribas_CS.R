...
FTSE100 <- read.csv2(files[1],sep=',', skip=0,header=TRUE,na='.',dec=".")%>%
    ...
    subset(date> "2006-05-01" & date < "2011-05-04") %>% group_by(date) %>%
...
SP500 <- read.csv2(files[2],skip=0, header=TRUE,sep=",",dec=".",na=".")%>%
    ...
    subset(date>"2006-05-01" & date < "2011-05-04") %>% group_by(date) %>%
    summarize(value,week) %>%
    mutate(return_ratios_daily=value/lag(value,1), 
    log_return_ratios_daily=log(return_ratios_daily)
    
joinedDataFrameDaily <- merge(SP500,FTSE100, by="date", 
suffixes=c('SP500','FTSE100')) %>%
group_by(date) %>% summarize(valueSP500, 
  return_ratios_dailySP500, 
  log_return_ratios_dailySP500, valueFTSE100, 
  return_ratios_dailyFTSE100, 
   log_return_ratios_dailyFTSE100,weekSP500)


joinedDataFrameWeekly <- joinedDataFrameDaily
!duplicated(joinedDataFrameDaily$weekSP500),]%>%
    mutate(return_ratios_weeklySP500=valueSP500/lag(valueSP500,1),
    log_return_ratios_weeklySP500=log(return_ratios_weeklySP500),
      return_ratios_weeklyFTSE100=valueFTSE100/ lag(valueFTSE100,1), 
      log_return_ratios_weeklyFTSE100= log(return_ratios_weeklyFTSE100))%>%
      group_by(weekSP500) %>% summarize(valueSP500,return_ratios_weeklySP500, 
      log_return_ratios_weeklySP500, valueFTSE100, return_ratios_weeklyFTSE100, log_return_ratios_weeklyFTSE100)
...
#There are about 252 trading dates per year. This is an assumption. Working weekly is easier. v_from_dailyData=colMeans(X)/(1/252)
#Using the fact that we can obtain MLE estimators for the parameters of the geometric Brownian motion by transforming the MLE estimators #calculated above, we can calculate MLE estimates for the mu's of both assets, as well as the sigmas. Let us continue working with our #weekly estimates, so let us define Sigma as Sigma_from_weeklyData %----------------------------------------------------------------------- Sigma=Sigma_from_weeklyData
