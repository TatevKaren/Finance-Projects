...
for j=1:S
    totalPayments=0;
     totalPremium=0;
    for i=1:N
        totalPayments=totalPayments+Y(tx(i,j),ty(i,j));
        totalPremium=totalPremium+PI(tx(i,j),ty(i,j));
    end
    Y_sim(j)=totalPayments/N;
    PI_sim(j)=totalPremium/N;
end

prob_xGetsPayed_sim=sum(sum(tx>ty & tx>=retX-x))/(S*N);

prob_yGetsPayed_sim=sum(sum(ty>tx & ty>=retY-y))/(S*N);

netPremium_sim=50000*(mean(Y_sim)/mean(PI_sim));

L_sim=50000*Y_sim-netPremium_sim*PI_sim;
stdL_sim=std(L_sim);

varPrinciplePremium_sim=netPremium_sim+0.5*alpha*var(L_sim)/mean(PI_sim);

L_varPremium_sim=50000*Y_sim-varPrinciplePremium_sim*PI_sim;
stdL_varPremium=std(L_varPremium);

U_sim=mean((1-exp(L_sim*alpha))/alpha);

U_varPremium_sim=mean((1-exp(L_varPremium_sim*alpha))/alpha);

P_netPremium_probOfDefault_sim=mean(L_sim>0);

P_VarPremium_probOfDefault_sim=mean(L_varPremium_sim>0);
